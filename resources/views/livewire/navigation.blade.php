...
<nav class="bg-gray-800" x-data="{ open:false}" >
   ...
   @auth
      <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
        ...
        <div class="relative ml-3" x-data="{open: false}"   >
          ...
          -->
          <div  x-show="open" style="display: none;" x-transition.duration.400ms x-on:click.away="open=false" class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
            ...
            <form action="{{route('logout')}}" method="POST" x-data>
              @csrf
            <a href="{{ route('logout') }}"
            @click.prevent="$root.submit();" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-2">Cerrar Sesión</a>
  ...
  <div class="sm:hidden" style="display: none;" id="mobile-menu" x-show="open" x-transition.duration.400ms x-on:click.away="open = false">
